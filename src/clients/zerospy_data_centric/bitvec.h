...
inline void bitvec_alloc(bitref_t bitref, size_t size) {
    ...
    if(size>BITVEC_PAGE_SIZE*64) {
        size_t capacity = CEIL(CEIL(size,64) + 1, BITVEC_PAGE_SIZE);
        // guaranttee to be zero
        bitref->data.dyn_pages = (uint64_t**)dr_raw_mem_alloc(capacity*sizeof(uint64_t*), DR_MEMPROT_READ | DR_MEMPROT_WRITE, NULL);
        assert(bitref->data.dyn_pages!=NULL);
        memset(bitref->data.dyn_pages, 0, capacity*sizeof(uint64_t*));
    } else if(size>64) {
        ...
        size_t capacity = CEIL(size,64) + 1;
        // bitref->capacity = (size+63)/64 + 1;
        //assert(bitref->capacity > 0);
        // Only Dynamic Malloc for large cases (>64 Bytes)
        bitref->data.dyn = (uint64_t*)dr_raw_mem_alloc(capacity*sizeof(uint64_t), DR_MEMPROT_READ | DR_MEMPROT_WRITE, NULL);
        assert(bitref->data.dyn!=NULL);
        memset(bitref->data.dyn, -1, capacity*sizeof(uint64_t));
...
inline void bitvec_free(bitref_t bitref) {
    if(bitref->size>64) {
        ...
        if(bitref->size>BITVEC_PAGE_SIZE*64) {
            capacity = CEIL(CEIL(bitref->size,64) + 1, BITVEC_PAGE_SIZE);
            ...
            capacity = CEIL(bitref->size,64) + 1;
        }
        dr_raw_mem_free(bitref->data.dyn, capacity*sizeof(uint64_t*));
    }
}
