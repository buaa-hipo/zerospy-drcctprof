...
private:
  ...
  inline void seekLower(K key, Node<K, V>** parent, Node<K, V>** node, Node<K, V>** injectionPoint);
...
public:
  ...
  ~ConcurrentTreeMap();
  void clear();
  Node<K, V>* lookup(const K key);
  Node<K, V>* lookup_lower_bound(const K key);
...
template<typename K, typename V>
void ConcurrentTreeMap<K, V>::clear()
{
  // quick clear
}
...
template<typename K, typename V>
inline void ConcurrentTreeMap<K, V>::seekLower(K key, Node<K, V>** parent, Node<K, V>** node, Node<K, V>** injectionPoint)
{
  Node<K, V>* prev[2];
  Node<K, V>* curr[2];
...
END:
  {
    ...
    // *parent = prev[index];
    if(lastRKey[cSeek] == std::numeric_limits<K>::min()) {
      //printf("SEEK LOWER: root right child\n");
      Node<K, V>* tmp = m_root->m_child[RIGHT];
      *node = tmp;
    } else {
      //printf("SEEK LOWER: lastRNode\n");
      *node = lastRNode[index];
    }
...
template<typename K, typename V>
Node<K, V>* ConcurrentTreeMap<K, V>::lookup(const K key)
{
  Node<K, V>* parent;
  Node<K, V>* node;
...
template<typename K, typename V>
Node<K, V>* ConcurrentTreeMap<K, V>::lookup_lower_bound(const K key)
{
  Node<K, V>* parent;
  Node<K, V>* node;
  Node<K, V>* injectionPoint;

  seekLower(key, &parent, &node, &injectionPoint);
  return node;
}
